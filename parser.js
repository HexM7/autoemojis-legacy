...
let substringArray = get_substrings_between(message.content, ":", ":");
    ...
    substringArray.forEach(m => {
        ...
        let emojiCheck = message.guild.emojis.cache.find(x => x.name === m);
        if (emoji && !emojiCheck) {
          checkPerm(client, message.channel, message.guild.id);
        };
        var replace = `:${m}:`;
...
async function checkPerm(client, channel, role) {
return;
  }
